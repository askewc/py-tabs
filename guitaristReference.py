...
FACTOR = 1.0
number_of_samples_to_process = int(
    8192 * FACTOR)  # 2^14 to make FFT work fast, and enough time for accurate reading, but not too long so we don't have too much spread
snd = 0
...
def voiceRecorder():
    ...
    while (not (is_finished())):
        ...
        for i in sound_events:
            ...
            print "%.2f\t%d\t%d\t%s\t%f" % (
            time, event_type, note_index, note_name, float(note_index) * sample_rate / number_of_samples_to_process)
        sound_events = []
...
def analyse_sound():
    ...
    if (peaks_only):
        ...
        for j0 in now_playing.keys():
            ...
            (time, last_power, max_power, last_area_power, max_area_power, was_decaying, detected_frames) = now_playing[
                j0]
            ...
            now_playing[j0] = (
            time, curr_power, max_power, last_area_power, max_area_power, was_decaying, detected_frames)
            ...
            if (curr_power < StopPower):
                ...
                print "%.2f: \t%s (%d) stopped playing at volume %d" % (
                cur_time, find_closest_note(j0 * sample_rate / N), j0, curr_power)
                # if(j0==110):
